...
\begin{table}[htbp]
\centering
\caption{8-class difficulty estimation across descriptor generations (mean $\pm$ std; RubricNet V2/V3 over 3 seeds $\times$ 5 folds, RubricNet V1 over a single seed, baselines over 5 folds). Bold marks the best value per generation.}
...
Decision Tree V2 & 0.286 $\pm$ 0.036 & 0.248 $\pm$ 0.031 & --- & 1.295 $\pm$ 0.118 & 3.127 $\pm$ 0.550 & 0.510 $\pm$ 0.058 \\
...
Decision Tree V3 & 0.279 $\pm$ 0.036 & 0.242 $\pm$ 0.034 & --- & 1.286 $\pm$ 0.059 & 3.130 $\pm$ 0.491 & 0.518 $\pm$ 0.040 \\
Random Forest V3 & \textbf{0.331 $\pm$ 0.022} & 0.289 $\pm$ 0.018 & --- & 1.122 $\pm$ 0.071 & 2.527 $\pm$ 0.312 & 0.599 $\pm$ 0.042 \\
...
Four observations stand out from the table.
...
The error structure behind these numbers is visible in Figure~\ref{fig:confusion}, which pools the RubricNet V3 test predictions of one seed across all five folds. The mass concentrates on the diagonal and its immediate neighbors --- 73\% of predictions fall within one bin of the truth --- and predictions far from the diagonal are rare, which is precisely the behavior the MAE and Kendall's $\tau$ figures summarize. The matrix also shows where the model still struggles: the easiest pieces (class 0) are most often placed one bin too high, and the middle classes blur into their neighbors, consistent with equal-frequency bins drawing hard boundaries through a continuum.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/confusion_matrix_v3.pdf}
    \caption{Confusion matrix of RubricNet V3 test predictions (seed 0, pooled over the five test folds; cell counts annotated, rows normalized by true-class size). Errors concentrate within one bin of the diagonal.}
    \label{fig:confusion}
\end{figure}
...
\midrule
Left hand only & 20 & 0.307 $\pm$ 0.049 & 0.261 $\pm$ 0.049 & 1.272 $\pm$ 0.066 & 3.158 $\pm$ 0.309 \\
Right hand only & 6 & 0.229 $\pm$ 0.033 & 0.203 $\pm$ 0.031 & 1.697 $\pm$ 0.061 & 5.171 $\pm$ 0.422 \\
Global only & 6 & 0.283 $\pm$ 0.021 & 0.261 $\pm$ 0.016 & 1.374 $\pm$ 0.081 & 3.579 $\pm$ 0.371 \\
All dimensions & 32 & \textbf{0.331 $\pm$ 0.022} & \textbf{0.289 $\pm$ 0.018} & \textbf{1.122 $\pm$ 0.071} & \textbf{2.527 $\pm$ 0.312} \\
\bottomrule
\end{tabular}
}
\end{table}
...
Figure~\ref{fig:rubric_example} makes this concrete with two test pieces from fold 0 at opposite ends of the scale: Sor's Andantino Op.\ 35 no.\ 2, a canonical beginner study (GuitarBurst level 1), and Bach's Chaconne from the second violin partita (level 19), a summit of the concert repertoire. The model places the Andantino in class 0 (correct) and the Chaconne in class 6 (true class 7), and the score sheets behind the two predictions read exactly as a teacher would expect: the Chaconne is pushed upward by its sheer length, sustained position-shift activity, and dense multi-voice texture, while the Andantino is pulled downward by its heavy open-string use, low fretboard positions, and narrow fretboard coverage. Notably, both pieces receive similar \emph{negative} contributions from the stretch descriptors --- wide chord stretches are simply not what distinguishes these two works --- illustrating that the rubric does not inflate every descriptor for a hard piece, but attributes the difficulty to the specific demands the score actually makes.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/rubric_example.pdf}
    \caption{Per-descriptor contributions $g_i(x_i)$ for an easy and a hard test piece (V3 model, fold 0), sorted by the Chaconne's contributions. The summed scores $S(x) = -3.06$ and $+5.66$ place the pieces in classes 0 and 6, respectively; every bar is one line of the rubric behind those predictions.}
...
The quantitative comparison in Section~\ref{sec:quantitative} attributed the fuzzy classifiers' deficit to their nominal treatment of the ordinal class structure, not to uninformative rules. Table~\ref{tab:fuzzy_rules} shows this directly: the five most relevant complete-search rules for the easiest class (0) and the hardest class (7), taken from fold 0.
...
To isolate rhythm's contribution, a reduced descriptor set was formed by removing all eight rhythm-aware V3 features (Section~\ref{sec:v3_features}), leaving the 24 non-rhythm descriptors of the V2 set. The Random Forest reached 0.325 accuracy on this reduced set (essentially unchanged from V3's 0.331, within the margin of imputation variance), while RubricNet dropped to 0.301 from 0.310. This indicates that the rhythm features, while not decisive for the ensemble model, provide a modest but genuine signal contribution to RubricNet that the more flexible model can substitute for from other inputs.

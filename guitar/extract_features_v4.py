"""
Phase A (v4): re-extract the v4 descriptor set for ALL 716 pieces from a single
unified source -- verified_pieces/all_xmls/ -- using the rhythm-aware timed parser.

This uses the corrected barre detector, position shift hand average calculation,
floor dt at 0.0625, and p95 max shift speed.

Output: features/guitar_descriptors_v4.csv (716 rows).
"""
import os
import sys
...
from guitar.guitar_features import get_timed_chords_from_xml_v4, calculate_descriptors_v4
...
def main():
    ...
    print(f"Re-extracting v4 base features for {len(df)} pieces from {ALL_XMLS}/ ...")
    ...
    for idx, row in df.iterrows():
        ...
        if path is None:
            ...
            seen[path] = idx
            chords, onsets, tempo, tech, has_rhythm = get_timed_chords_from_xml_v4(path)
            if chords is None:
                n_fail += 1
                print(f"  [PARSE-FAIL] row {idx}: {os.path.basename(path)}")
            else:
                feats = calculate_descriptors_v4(chords, onsets, has_rhythm)
    ...
    out.to_csv("features/guitar_descriptors_v4.csv", index=False)
    print(f"\nWrote features/guitar_descriptors_v4.csv ({len(out)} rows)")
    ...
    # Spearman table (v4 vs difficulty).
    ...
    print("\nSpearman |rho| vs Difficulty (v4 features):")
    for c, r in corr:
